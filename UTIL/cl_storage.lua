...
RegisterCommand('lvcfactoryreset', function(source, args)
	...
	if choice then
		...
		HUD:ShowNotification("~g~Success~s~: You have deleted all save data and reset LVC.")
	end
end)
------------------------------------------------
...
function Storage:SaveSettings()
	...
	--HUD Settings
	local hud_save_data = { Show_HUD = HUD:GetHudState(), HUD_Scale = HUD:GetHudScale(), HUD_pos = HUD:GetHudPosition() }
	...
	if UTIL:GetVehicleProfileName() ~= nil then
		...
		if profile_name ~= nil then
			...
			HUD:ShowNotification("~b~LVC: ~r~ SAVE ERROR~s~: profile_name after gsub is nil.")
		end
	else
	...
	if save_version ~= nil then
		...
		if UTIL:GetVehicleProfileName() ~= nil then
			...
			if profile_name ~= nil then
				...
				HUD:ShowNotification("~b~LVC:~r~ LOADING ERROR~s~: profile_name after gsub is nil.")
			end
		end
	end
end
